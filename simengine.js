...
// simengine.js 내부의 getAvailableEvent 함수를 아래로 교체
function getAvailableEvent(year, key) {
    if (!gameState || !gameState.completedEvents) return null;
    
    // 만약 해당 연도 기록이 없으면 강제로 생성 (에러 방지)
    if (!gameState.completedEvents[year]) {
        gameState.completedEvents[year] = { chat: [], alba: [], home: [], special: [] };
    }
    ...
    if (!yearObj || !yearObj.events) return null;
    
    const events = yearObj.events[key];
...
function enterLocation(key) {
    ...
    // 플레이어 이름을 이벤트 상황 텍스트에 동적 반영
    const customSituation = (event.situation || "").replaceAll("친구", gameState.playerName);

    if (event.type === 'chat') {
        container.style.display = 'none';
        document.getElementById('chat-panel').style.display = 'block';
        document.getElementById('situation-text').innerText = customSituation;
        const customChat = (event.chatMessage || "").replaceAll("친구", gameState.playerName);
        document.getElementById('kakao-chat-content').innerText = customChat;
        return;
    }
...
function pickFollowupOptions() {
    // 👉 해당 이벤트에 맞춤 단계별 대화(steps)가 정의되어 있다면 그 단계의 선택지를 반환합니다.
    if (activeEventData && activeEventData.steps && activeEventData.steps[tikitakaStep]) {
        return activeEventData.steps[tikitakaStep];
    }
...
// 👉 아래와 같이 function finishTurnAction() { ... } 으로 코드를 감싸주세요.
function finishTurnAction() {
    if (gameState.dhEsteem <= 15) {
        showModal("⚠️ 위험 경고", `${gameState.playerName}야, 동현이의 자존감이 너무 바닥을 치고 있어! 이러다 큰일 나겠어.`);
        return;
    }
    if (gameState.dhEsteem >= 85) {
        showModal("⚠️ 위험 경고", `${gameState.playerName}야, 동현이의 자존감이 하늘을 찔러 통제 불능이야! 주의해.`);
        return;
    }

    if (gameState.intimacy <= 15) {
        showModal("⚠️ 손절 위기", `동현이와의 친밀도가 너무 낮아 손절당하기 직전이야!`);
        return;
    }
    if (gameState.intimacy >= 90) {
        showModal("⚠️ 기생충 위험", `친밀도가 너무 높아 동현이가 완전히 빌붙으려 한다!`);
        return;
    }
